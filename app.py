...
def load_tx_data():
    ...
    def create_news_url(row):
        ...
        return f"https://www.google.com/search?btnI=1&q={q}"
...
selected_regions = st.sidebar.multiselect("Select Regions:", list(GEO_COORDS.keys()), default=list(GEO_COORDS.keys()))
...
with tab1:
    ...
    with c1: 
        ...
        st.plotly_chart(px.scatter_geo(filtered_df, lat='lat', lon='lon', color='Region', size='Capacity_MW', hover_name='Asset', projection="natural earth", scope="asia", title="Bubble Size = MW").update_layout(margin=dict(l=0,r=0,t=30,b=0)), use_container_width=True)
    with c2: 
        st.markdown("**Investment Volume**")
        st.plotly_chart(px.bar(filtered_df.groupby('Region')['Consideration_USD_M'].sum().reset_index(), x='Region', y='Consideration_USD_M', color='Region', template='plotly_white').update_layout(showlegend=False, xaxis_title="", yaxis_title="USD (Millions)"), use_container_width=True)
...
with tab2:
    ...
    st.dataframe(display_df[display_df['Status'] == 'Executed'], column_config=col_formatting, hide_index=True, use_container_width=True)

    st.subheader("⏳ Tentative & Pipeline Deals")
    st.dataframe(display_df[display_df['Status'] == 'Tentative'], column_config=col_formatting, hide_index=True, use_container_width=True)
